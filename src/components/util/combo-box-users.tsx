...
import { GetUserList } from "@/lib/service/users-service";
...
export function ComboboxUsers({ value, onChange, className }: ComboboxProps) {
  const [dataList, setDataList] = useState<Users[]>([]);
  const [displayedData, setDisplayedData] = useState<Users[]>([]);
  ...
  const getFilteredDatas = () => {
    ...
    return dataList.filter((data) =>
      data.employee?.fullName?.toLowerCase().includes(searchTerm.toLowerCase())
    );
  };
                  ...
                  {displayedData.map((data) => (
                    ...
                    >
                      <span className="flex-1">
                        {data.employee?.nip} - {data.employee?.fullName}
                      </span>
                      <Check
                        className={cn(
                          "ml-auto h-4 w-4",
