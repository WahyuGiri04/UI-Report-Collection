    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-start justify-start space-x-2">
          ...
          <Badge
            variant={row.original.mainMenuId?.menuType === 1 ? "outline" : "secondary"}
            className="flex gap-1 px-1.5 text-muted-foreground [&_svg]:size-3"
          >
            <SafeDynamicIcon name={row.original.mainMenuId?.menuType === 1 ? "layout-dashboard" : "menu"} />
            <span>{row.original.mainMenuId?.menuType === 1 ? "Dashboard" : "Menu"}</span>
          </Badge>
        </div>
      ),
    },
