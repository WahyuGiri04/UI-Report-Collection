...
export async function GetMenuPage(
    ...
    const token = GetToken()
    const response = await fetch(`${API_BASE_URL}admin/main-menu/search?mainMenuName=${menuName}&menuType=${menuType}&page=${pageNumber}&pageSize=${rowPerPage}`,
        {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
...
export async function UpdateMenu(data: Menu, id: number): Promise<BaseResponse<Menu>> {
    const token = GetToken()
